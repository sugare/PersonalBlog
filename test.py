...
集群是一种并行或分布式系统，该系统包括一个互连的整体计算机集合作为一种单一、统一的计算机资源使用。通过集群技术，我们可以在付出低成本的情况下获得在性能、可靠性、灵活性方面更高的利益。
计算机集群简称集群，是一组计算机系统，它通过一组松散集成的计算机软件和硬件连接起来，高度紧密地协作完成计算相关工作。
集群，是指一组相互独立的计算机，利用高速通信网络组成的一个计算机系统，每个集群节点（即集群中的每台计算机）都是运行其在即进程的一个独立服务器。这些进程可以彼此通信，对网络客户机来说就像形成了一个单一系统，协同起来向用户提供应用程序、系统资源和数据，并以单一系统的模式加以管理。一个客户与集群相互作用时，集群像是一个独立的服务器。
###集群的基本特点
1.高性能
...
4.高可靠性
集群技术使系统在故障发生时扔可以继续工作，将系统停机时间减到最小。
###集群的分类
负载均衡集群（LB）
...
2）保证业务连续7*24提供业务
"""
sql = """insert into record(title, date, essay, view) values('22222','2017-01-04','%s','4');""" %MySQLdb.escape_string(d)
cursor.execute(sql)
db.commit()
